...
CREATE TABLE lennujaam (
    ...
    reg_aeg TIMESTAMPTZ(0) NOT NULL DEFAULT CURRENT_TIMESTAMP(0),
    viimase_muutm_aeg TIMESTAMPTZ(0) NOT NULL DEFAULT CURRENT_TIMESTAMP(0),
    ...
    CONSTRAINT chk_lennujaam_aeg_order CHECK (viimase_muutm_aeg IS NULL OR (viimase_muutm_aeg >= reg_aeg AND viimase_muutm_aeg < (CURRENT_TIMESTAMP(0)+'1 second'::interval)))
);
...
CREATE TABLE lennukituup (
    ...
    CONSTRAINT chk_lennukituup_max_reisijate_arv_positive CHECK (maksimaalne_reisijate_arv > 0),
    CONSTRAINT chk_lennukituup_pardapersonali_arv_positive CHECK (pardapersonali_arv > 0),
    CONSTRAINT chk_lennukituup_pilootide_arv_positive CHECK (pilootide_arv > 0)
);
...
CREATE TABLE isik (
    ...
    CONSTRAINT chk_isik_e_meil_not_empty CHECK (e_meil !~ '^[[:space:]]*$'),
    CONSTRAINT chk_isik_email_format CHECK (e_meil LIKE '_%@_%.__%'),
    CONSTRAINT chk_isik_synni_kp_range CHECK (synni_kp >= '1900-01-01' AND synni_kp < CURRENT_DATE),
    CONSTRAINT chk_isik_reg_aeg_range CHECK (reg_aeg >= '2000-01-01' AND reg_aeg < '2100-01-01'),
    CONSTRAINT chk_isik_aeg_order CHECK (viimase_muutm_aeg IS NULL OR (viimase_muutm_aeg >= reg_aeg AND viimase_muutm_aeg < (CURRENT_TIMESTAMP(0)+'1 second'::interval))),
    CONSTRAINT chk_isik_eesnimi_or_perenimi_not_null CHECK (eesnimi IS NOT NULL OR perenimi IS NOT NULL),
    CONSTRAINT chk_isik_reg_aeg_after_synni_kp CHECK (synni_kp IS NULL OR reg_aeg::date > synni_kp)
);
...
CREATE TABLE kuupaevade_vahemiku_maandumiskeeld (
    ...
    CONSTRAINT chk_kvm_alguse_aeg_range CHECK (alguse_aeg >= '2000-01-01' AND alguse_aeg < '2100-01-01'),
    CONSTRAINT chk_kvm_lopu_aeg_range CHECK (lopu_aeg >= '2000-01-01' AND lopu_aeg < '2100-01-01'),
    CONSTRAINT chk_kvm_aeg_order CHECK (lopu_aeg > alguse_aeg)
);
...
CREATE TABLE lend (
    ...
    CONSTRAINT chk_lend_tegelik_lahkumis_aeg_range CHECK (tegelik_lahkumis_aeg IS NULL OR (tegelik_lahkumis_aeg >= reg_aeg AND tegelik_lahkumis_aeg < (CURRENT_TIMESTAMP(0)+'1 second'::interval))),
    ...
    CONSTRAINT chk_lend_viimase_muutm_aeg_range CHECK (viimase_muutm_aeg IS NULL OR (viimase_muutm_aeg >= reg_aeg AND viimase_muutm_aeg < (CURRENT_TIMESTAMP(0)+'1 second'::interval)))
);
...
CREATE TABLE hooldus (
    ...
    CONSTRAINT chk_hooldus_algus_range CHECK (alguse_aeg >= '2000-01-01' AND alguse_aeg < (CURRENT_TIMESTAMP(0)+'5 years'::interval)),
    CONSTRAINT chk_hooldus_lopp_range CHECK (lopu_aeg >= alguse_aeg AND lopu_aeg < (alguse_aeg + '6 months'::interval))
);
...
CREATE TABLE broneering (
    ...
    seisund_kood VARCHAR(20) NOT NULL DEFAULT 'ACTIVE',
    broneerimise_aeg TIMESTAMPTZ(0) NOT NULL DEFAULT CURRENT_TIMESTAMP(0),
    viimase_muutm_aeg TIMESTAMPTZ(0) NOT NULL DEFAULT CURRENT_TIMESTAMP(0),
    ...
    CONSTRAINT chk_broneering_viimase_muutm_aeg_range CHECK (viimase_muutm_aeg IS NULL OR (viimase_muutm_aeg >= broneerimise_aeg AND viimase_muutm_aeg < (CURRENT_TIMESTAMP(0)+'1 second'::interval))),
...
CREATE INDEX idx_kuup_vahemiku_maandumiskeeld_lennujaam_aeg ON kuupaevade_vahemiku_maandumiskeeld(lennujaam_kood, alguse_aeg, lopu_aeg);
...
BEGIN
    IF EXISTS (
        ...
        WHERE lennujaam_kood = p_airport_code
          AND TSTZRANGE(alguse_aeg, lopu_aeg, '[]') @> p_arrival_time
    ) THEN
...
CREATE OR REPLACE FUNCTION lennufirma.fn_check_airport_status(
    p_airport_code VARCHAR(3)
) RETURNS VOID LANGUAGE plpgsql AS $$
DECLARE
    v_status VARCHAR(20);
BEGIN
    SELECT seisund_kood INTO v_status FROM lennufirma.lennujaam WHERE lennujaam_kood = p_airport_code;
...
END;
$$;
COMMENT ON FUNCTION lennufirma.fn_check_airport_status IS 'Checks if an airport status is OPEN.';

-- Helper function to check aircraft status
CREATE OR REPLACE FUNCTION lennufirma.fn_check_aircraft_status(
    p_aircraft_reg_nr VARCHAR(10)
) RETURNS VOID LANGUAGE plpgsql AS $$
DECLARE
    v_status VARCHAR(20);
BEGIN
    SELECT seisund_kood INTO v_status FROM lennufirma.lennuk WHERE registreerimisnumber = p_aircraft_reg_nr;
...
END;
$$;
COMMENT ON FUNCTION lennufirma.fn_check_aircraft_status IS 'Checks if an aircraft status is ACTIVE.';
...
RETURNS VOID LANGUAGE plpgsql AS $$
DECLARE v_lend_seisund VARCHAR(20); v_original_lahkumis_aeg TIMESTAMPTZ; v_siht_kood VARCHAR(3);
BEGIN
...
DECLARE
  ...
  v_final_lahkumis_aeg TIMESTAMPTZ; v_final_saabumis_aeg TIMESTAMPTZ;
...
BEGIN
  ...
  PERFORM fn_check_airport_status(v_final_lahke_kood);
  PERFORM fn_check_airport_status(v_final_siht_kood);
  PERFORM fn_check_landing_ban(v_final_siht_kood, v_final_saabumis_aeg);
  IF v_final_reg_nr IS NOT NULL THEN
      PERFORM fn_check_aircraft_status(v_final_reg_nr);
  END IF;

  -- Final Airport Type Compatibility Checks
  IF v_final_tuup_kood IS NOT NULL THEN
    SELECT EXISTS (SELECT 1 FROM lennufirma.lennujaama_vastuvoetavad_lennukituubid llvt WHERE llvt.lennujaam_kood = v_final_lahke_kood AND llvt.lennukituup_kood = v_final_tuup_kood) INTO v_aircraft_type_supported_departure;
    ...
    SELECT EXISTS (SELECT 1 FROM lennufirma.lennujaama_vastuvoetavad_lennukituubid llvt WHERE llvt.lennujaam_kood = v_final_siht_kood AND llvt.lennukituup_kood = v_final_tuup_kood) INTO v_aircraft_type_supported_arrival;
...
BEGIN
  ...
  SELECT EXISTS (
      ...
      JOIN lend l_conflict ON tl.lend_kood = l_conflict.lend_kood
...
-- Core Entities
INSERT INTO lennujaam (lennujaam_kood, nimi, koordinaadid_laius, koordinaadid_pikkus, seisund_kood) VALUES
...
INSERT INTO lennukituup (lennukituup_kood, nimetus, maksimaalne_lennukaugus, maksimaalne_reisijate_arv, pardapersonali_arv, pilootide_arv) VALUES
...
-- Sample Landing Ban
INSERT INTO kuupaevade_vahemiku_maandumiskeeld (lennujaam_kood, alguse_aeg, lopu_aeg) VALUES
